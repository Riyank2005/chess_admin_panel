...
export function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#050510] flex selection:bg-cyan-500/20 font-outfit">
      <BroadcastBanner />
      ...
      <main className={cn(
        ...
        <div className="flex justify-between items-center gap-4 mb-6 sticky top-0 z-30 pointer-events-none">
          ...
          <div className="flex items-center gap-3 pointer-events-auto backdrop-blur-md bg-black/20 p-2 rounded-2xl border border-white/5">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl border bg-black/40 transition-all border-white/5">
              <div className={cn("h-2 w-2 rounded-full animate-pulse", maintenanceMode ? "bg-rose-500 shadow-[0_0_10px_#f43f5e]" : "bg-emerald-500 shadow-[0_0_10px_#10b981]")}></div>
              <span className={cn("text-[10px] font-bold tracking-wider", maintenanceMode ? "text-rose-400" : "text-emerald-400")}>{maintenanceMode ? "MAINTENANCE" : "ONLINE"}</span>
            </div>
            <ModeToggle />
          </div>
        </div>
        ...
        <div className={cn("mt-12 flex justify-between items-center text-[10px] text-white/30 uppercase tracking-widest font-bold", isMobile ? "flex-col gap-4 items-center" : "")}>
          <div className="flex items-center gap-4">
            <span>Chess Master v3.0</span>
            <span className="text-white/10">•</span>
            <span className="text-emerald-500/50">System Stable</span>
          </div>
          <div className="flex items-center gap-6">
            <LegalDialog type="terms" trigger={<button className="hover:text-cyan-400 transition-colors">Terms</button>} />
            <LegalDialog type="privacy" trigger={<button className="hover:text-cyan-400 transition-colors">Privacy</button>} />
          </div>
        </div>
      </main>
    </div>
